 ...
 */
public class JacksonJsonParserService<T> implements JsonParser<T> {
    private final ObjectMapper mapper = new ObjectMapper()
            .configure(DeserializationFeature.FAIL_ON_UNKNOWN_PROPERTIES, false)
            .configure(DeserializationFeature.FAIL_ON_TRAILING_TOKENS, true);
    private final JsonFactory factory = mapper.getFactory();

    public Stream<T> streamTargets(File jsonFile, Class<T> targetClass) throws IOException {
        com.fasterxml.jackson.core.JsonParser parser = factory.createParser(jsonFile);
